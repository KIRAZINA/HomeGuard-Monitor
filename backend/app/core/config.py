...
class Settings(BaseSettings):
    ...
    METRICS_RETENTION_DAYS: int = Field(
        default=30, ge=1, description="Metrics retention period in days"
    )
    ALERT_EVALUATION_INTERVAL_SECONDS: int = Field(
        default=60, ge=10, description="Alert evaluation interval in seconds"
    )
    ALERT_COOLDOWN_SECONDS: int = Field(
        default=300, ge=10, description="Alert cooldown period in seconds"
    )
