...
class RateLimiter:
    ...
    def _get_recent_requests(self, client_id: str, since: datetime) -> List[datetime]:
        ...
        """
        return [t for t in self._windows[client_id] if t > since]
    ...
    def get_limiter(self, endpoint: str) -> RateLimiter:
        ...
        if endpoint not in self._limiters:
            ...
            if "/auth/login" in endpoint:
                self._limiters[endpoint] = RateLimiter(requests_per_minute=10, burst_size=3)
            elif "/auth/register" in endpoint:
                self._limiters[endpoint] = RateLimiter(requests_per_minute=20, burst_size=5)
            elif "/metrics/ingest" in endpoint:
                self._limiters[endpoint] = RateLimiter(requests_per_minute=100, burst_size=20)
            else:
            ...
            return JSONResponse(
                ...
                content={
                    "error": {
                        ...
                        "details": {"retry_after": "Please wait before making another request"},
                    }
                },
            )
