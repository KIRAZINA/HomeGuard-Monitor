...
class MetricService:
    ...
    async def get_metrics(self, query: MetricQuery) -> List[Metric]:
        ...
        conditions.append(
            and_(Metric.timestamp >= query.start_time, Metric.timestamp <= query.end_time)
        )
    ...
    async def get_metrics_summary(self, device_id: int, hours: int = 24) -> List[MetricSummary]:
        ...
        stmt = (
            ...
            .where(and_(Metric.device_id == device_id, Metric.timestamp >= start_time))
        ...
        summaries = []
        for row in rows:
            latest_metric = await self.get_latest_metric_value(device_id, row.metric_type)
