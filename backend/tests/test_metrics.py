...
class TestMetrics:
    ...
    async def test_ingest_metrics_unauthorized(self, client: AsyncClient, test_device):
        ...
        metrics_data = [
            {"device_id": test_device.id, "metric_type": "cpu_usage_percent", "value": 75.5}
        ]
    ...
    async def test_ingest_metrics_invalid_device(self, client: AsyncClient, auth_headers):
        ...
        metrics_data = [{"device_id": 99999, "metric_type": "cpu_usage_percent", "value": 75.5}]
    ...
    async def test_get_metrics_success(self, client: AsyncClient, auth_headers, test_metrics):
        ...
        response = await client.get(
            f"/api/v1/metrics/?device_id={device_id}&limit=5", headers=auth_headers
        )
    ...
    async def test_get_metrics_with_time_range(
        self, client: AsyncClient, auth_headers, test_metrics
    ):
    ...
    async def test_get_latest_metrics_success(self, client: AsyncClient, auth_headers, test_device):
        ...
        response = await client.get(
            f"/api/v1/metrics/device/{test_device.id}/latest", headers=auth_headers
        )
    ...
    async def test_get_latest_metrics_device_not_found(self, client: AsyncClient, auth_headers):
        ...
        response = await client.get("/api/v1/metrics/device/99999/latest", headers=auth_headers)
    ...
    async def test_get_metrics_summary_custom_hours(
        self, client: AsyncClient, auth_headers, test_device
    ):
        """Test getting metrics summary with custom time range."""
        response = await client.get(
            f"/api/v1/metrics/device/{test_device.id}/summary?hours=48", headers=auth_headers
        )
    ...
    async def test_ingest_metrics_with_timestamp(
        self, client: AsyncClient, auth_headers, test_device
    ):
    ...
    async def test_get_metrics_limit(self, client: AsyncClient, auth_headers, test_metrics):
        ...
        response = await client.get(
            f"/api/v1/metrics/?device_id={device_id}&limit=3", headers=auth_headers
        )
