...
class TestAlerts:
    ...
    async def test_create_alert_rule_success(self, client: AsyncClient, auth_headers, test_device):
        ...
        response = await client.post("/api/v1/alerts/rules", json=rule_data, headers=auth_headers)
    ...
    async def test_update_alert_rule_success(self, client: AsyncClient, auth_headers, test_device):
        ...
        create_response = await client.post(
            "/api/v1/alerts/rules", json=rule_data, headers=auth_headers
        )
        rule_id = create_response.json()["id"]

        # Now update it
        update_data = {"name": "Updated Rule", "threshold_value": 90.0, "severity": "critical"}

        response = await client.put(
            f"/api/v1/alerts/rules/{rule_id}", json=update_data, headers=auth_headers
        )
    ...
    async def test_update_alert_rule_not_found(self, client: AsyncClient, auth_headers):
        ...
        response = await client.put(
            "/api/v1/alerts/rules/99999", json=update_data, headers=auth_headers
        )
    ...
    async def test_delete_alert_rule_success(self, client: AsyncClient, auth_headers, test_device):
        ...
        # Now delete it
        response = await client.delete(f"/api/v1/alerts/rules/{rule_id}", headers=auth_headers)
    ...
    async def test_get_alerts_with_filters(self, client: AsyncClient, auth_headers):
        ...
        response = await client.get("/api/v1/alerts/?acknowledged=false", headers=auth_headers)
        ...
        response = await client.get("/api/v1/alerts/?acknowledged=true", headers=auth_headers)
    ...
    async def test_acknowledge_alert_success(self, client: AsyncClient, auth_headers):
        ...
        response = await client.post("/api/v1/alerts/1/acknowledge", headers=auth_headers)
    ...
    async def test_acknowledge_alert_not_found(self, client: AsyncClient, auth_headers):
        ...
        response = await client.post("/api/v1/alerts/99999/acknowledge", headers=auth_headers)

        assert response.status_code == 404

    async def test_snooze_alert_success(self, client: AsyncClient, auth_headers):
        """Test snoozing an alert."""
        response = await client.post("/api/v1/alerts/1/snooze?minutes=30", headers=auth_headers)
    ...
    async def test_snooze_alert_not_found(self, client: AsyncClient, auth_headers):
        ...
        response = await client.post("/api/v1/alerts/99999/snooze?minutes=30", headers=auth_headers)
    ...
    async def test_alert_rule_types(self, client: AsyncClient, auth_headers, test_device):
        ...
        response = await client.post(
            "/api/v1/alerts/rules", json=threshold_rule, headers=auth_headers
        )
        assert response.status_code == 200
        ...
        response = await client.post(
            "/api/v1/alerts/rules", json=anomaly_rule, headers=auth_headers
        )
        assert response.status_code == 200
