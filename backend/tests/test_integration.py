...
class TestIntegration:
    ...
    async def test_complete_monitoring_workflow(self, client: AsyncClient, auth_headers):
        ...
        device_response = await client.post(
            "/api/v1/devices/", json=device_data, headers=auth_headers
        )
        ...
        metrics_response = await client.post(
            "/api/v1/metrics/ingest", json=metrics_data, headers=auth_headers
        )
        ...
        stored_metrics_response = await client.get(
            f"/api/v1/metrics/?device_id={device_id}", headers=auth_headers
        )
        ...
        latest_response = await client.get(
            f"/api/v1/metrics/device/{device_id}/latest", headers=auth_headers
        )
        ...
        summary_response = await client.get(
            f"/api/v1/metrics/device/{device_id}/summary", headers=auth_headers
        )
        ...
        update_response = await client.put(
            f"/api/v1/devices/{device_id}", json={"status": "online"}, headers=auth_headers
        )
    ...
    async def test_multi_device_monitoring(self, client: AsyncClient, auth_headers):
        ...
        for device in devices:
            metrics_response = await client.get(
                f"/api/v1/metrics/?device_id={device['id']}", headers=auth_headers
            )
    ...
    async def test_metrics_time_range_queries(self, client: AsyncClient, auth_headers):
        ...
        time_ranges = [("1 hour", 1), ("2 hours", 2), ("6 hours", 6)]
    ...
    async def test_error_handling_workflow(self, client: AsyncClient):
        ...
        auth_response = await client.post(
            "/api/v1/auth/register", json={"email": "invalid-email", "password": "123"}  # Too short
        )
        ...
        login_response = await client.post(
            "/api/v1/auth/login",
            data={"username": "error-test@example.com", "password": "testpassword123"},
        )
    ...
    async def test_performance_with_large_dataset(self, client: AsyncClient, auth_headers):
        ...
        response = await client.post(
            "/api/v1/metrics/ingest", json=large_metrics_batch, headers=auth_headers
        )
        ...
        response = await client.get(
            f"/api/v1/metrics/?device_id={device_id}&limit=100", headers=auth_headers
        )
        ...
        # Test pagination
        response = await client.get(
            f"/api/v1/metrics/?device_id={device_id}&limit=50&skip=50", headers=auth_headers
        )
