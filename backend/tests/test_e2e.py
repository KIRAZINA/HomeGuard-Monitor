...
class TestE2EDeviceMonitoring:
    ...
    @pytest.mark.e2e
    async def test_complete_device_monitoring_workflow(
        ...
        metrics_data = [
            MetricCreate(
                device_id=device_id, metric_type="cpu_usage_percent", value=75.0, unit="percent"
            ),
            MetricCreate(
                device_id=device_id, metric_type="memory_usage_percent", value=60.0, unit="percent"
            ),
            MetricCreate(
                device_id=device_id, metric_type="disk_usage_percent", value=45.0, unit="percent"
            ),
        ]
...
class TestE2EAlertWorkflow:
    ...
    async def test_alert_acknowledge_workflow(self, client: AsyncClient, auth_headers, db_session):
        ...
        device = await device_service.create_device(
            DeviceCreate(
                name="Alert Test Device", device_type="server", hostname="alert-test.local"
            )
        )
    ...
    async def test_alert_snooze_workflow(self, client: AsyncClient, auth_headers, db_session):
        ...
        device = await device_service.create_device(
            DeviceCreate(
                name="Snooze Test Device", device_type="server", hostname="snooze-test.local"
            )
        )
...
class TestE2EMetricsAggregation:
    ...
    @pytest.mark.e2e
    async def test_metrics_time_range_aggregation(
        self, client: AsyncClient, auth_headers, db_session
    ):
        """
        ...
        metrics_1h = await metric_service.get_metrics(
            device_id=device.id, start_time=now - timedelta(hours=1), end_time=now
        )

        # 2 hour range
        metrics_2h = await metric_service.get_metrics(
            device_id=device.id, start_time=now - timedelta(hours=2), end_time=now
        )
...
class TestE2ENotificationChannels:
    ...
    async def test_multi_channel_notification(self, client: AsyncClient, auth_headers, db_session):
        ...
        device = await device_service.create_device(
            DeviceCreate(
                name="Multi-Channel Device", device_type="server", hostname="multi-channel.local"
            )
        )
        ...
        with patch.object(
            notification_service, "_send_email_notification", new_callable=AsyncMock
        ) as mock_email:
            with patch.object(
                notification_service, "_send_telegram_notification", new_callable=AsyncMock
            ) as mock_telegram:
                with patch.object(
                    notification_service, "_send_discord_notification", new_callable=AsyncMock
                ) as mock_discord:
...
class TestE2EDeviceLifecycle:
    ...
    async def test_device_crud_lifecycle(self, client: AsyncClient, auth_headers, db_session):
        ...
        updated_device = await device_service.update_device(
            device_id,
            DeviceUpdate(name="Updated Lifecycle Device", description="Updated description"),
        )
...
class TestE2EAuthenticationFlow:
    ...
    async def test_complete_auth_flow(self, client: AsyncClient):
        ...
        # 2. Login
        login_data = {"username": unique_email, "password": "e2epassword123"}
...
class TestE2EErrorHandling:
    ...
    async def test_graceful_error_handling(self, client: AsyncClient):
        ...
        invalid_data_response = await client.post(
            ...
            json={"email": "not-an-email", "password": "123", "full_name": "Test"},  # Too short
        )
